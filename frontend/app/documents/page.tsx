...
import { uploadDocument, getDocuments, getOcrStatus } from '../services/documents';
import { useSemanticAnalysis, useArgumentAnalysis } from './hooks';
...
export default function DocumentManagementPage() {
  ...
  // Analysis hooks (import from hooks.ts)
  const { data: semanticData, isLoading: semanticLoading } = useSemanticAnalysis(documentToView?.id, { enabled: !!documentToView && isAnalysisModalOpen });
  const { data: argumentData, isLoading: argumentLoading } = useArgumentAnalysis(documentToView?.id, { enabled: !!documentToView && isAnalysisModalOpen });

  // Fetch documents
  const { data: documents, isLoading, isError } = useQuery<Document[]>(['documents'], () => getDocuments(session));

  // Upload mutation
  const uploadMutation = useMutation(
    (formData: FormData) => uploadDocument(formData, session),
    {
  ...
  const handleViewOcr = async (doc: Document) => {
    ...
    try {
      const result = await getOcrStatus(doc.id, session);
      setOcrResult(result);
    } catch (e) {
      setOcrResult({ status: 'Error', result: null });
    }
    setOcrLoading(false);
  ...
  // Pagination logic
  const paginatedDocs = documents ? documents.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage) : [];
  ...
  return (
    <div className="container mx-auto py-8">
              ...
              {paginatedDocs.map((doc: Document) => (
                <tr key={doc.id}>
                  ...
                  <td>
                    ...
                    <button className="btn btn-sm btn-info" onClick={() => handleViewOcr(doc)}>
                      View OCR
                    </button>
                    <button className="btn btn-sm btn-secondary ml-2" onClick={() => { setDocumentToView(doc); setShowAnalysis(true); setIsViewModalOpen(true); }}>
                      View Analysis
                    </button>
                  </td>
                </tr>
      ...
      {documents && (
        <div className="flex justify-between items-center mt-4">
          ...
          <div>
            ...
            <span className="mx-2">Page {currentPage} of {Math.ceil(documents.length / itemsPerPage)}</span>
            <button
              className="btn btn-sm"
      ...
      {/* OCR & Analysis Modal */}
      <Modal isOpen={isViewModalOpen} onClose={() => { setIsViewModalOpen(false); setShowAnalysis(false); }}>
        {showAnalysis && documentToView ? (
          <div>
            <h3 className="font-bold mb-2">AI Analysis for: {documentToView.title}</h3>
            {/* Semantic Analysis */}
            <div className="mb-4">
              <h4 className="font-semibold">Semantic Topics/Themes</h4>
              {semanticLoading ? (
                <div>Loading semantic analysis...</div>
              ) : (
                <ul className="list-disc ml-6">
                  {(semanticData?.topics as string[] | undefined)?.map((topic: string, idx: number) => (
                    <li key={idx}>{topic}</li>
                  ))
                }
              </ul>
            ...
            {/* Argument Analysis */}
            <div>
              <h4 className="font-semibold">Extracted Arguments</h4>
              {argumentLoading ? (
                <div>Loading argument analysis...</div>
              ) : (
                <ul className="list-disc ml-6">
                  {argumentData.arguments?.map((arg: any, idx: number) => (
                    <li key={idx}>{arg.text}</li>
                  ))}
                </ul>
              )}
            </div>
